	...
	const mealSections = [
		...
		...weekdays.map((day) => {
			...
			const filteredMeals = meals.filter((meal) =>
				meal.plannedAt
					? new Date(meal.plannedAt).toDateString() === dateKey
					: false,
			);
			return {
		...
		<div className="flex h-full w-full grow flex-col py-1.5">
			...
			<Tabs
				...
				classNames={{
					...
					tabWrapper: "h-full",
					tabList:
						"flex h-12 w-full flex-auto items-start gap-1 overflow-x-hidden shadow-small",
					panel:
						"!aspect-auto !w-full flex max-w-full grow flex-col justify-between pt-1",
				}}
			>
				{(item) => (
					<Tab
						...
						title={item.label}
						className={cn(
							item.index < 2 && "aspect-square w-fit px-0",
							"h-full",
						)}
					>
						<TabContent>
										...
										>
											{section.items.map((meal) => (
												<ListboxItem
													...
													endContent={
														meal ? (
															<MagnifyingGlassIcon className="cursor-pointer text-secondary" />
														) : (
															<PlusIcon className="cursor-pointer text-primary" />
														)
													}
												>
